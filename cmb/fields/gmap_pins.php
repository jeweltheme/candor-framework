...
wp_localize_script( 'gmap_pins', 'l18n_gmap_pins', array(
	'location_url_label' => __('Location URL', 'candor'),
	'name_label' => __('Name', 'candor'),
	'delete_label' => __('Delete', 'candor'),
	'confirm_delete' => __('Are you sure?', 'candor'),
	'dont_delete_all_pins' => __('This page is useless without pins.Better delete the page!', 'candor'),
) );
...
<div class="gmap_pins_container">
		...
		foreach ( $meta as $key => $pin ) { ?>
			<li class="gmap_pin">
				<fieldset class="pin_location_url">
					<label for="<?php echo $field['id']; ?>[<?php echo $key ?>][location_url]" >#<?php echo $key . ' ' . __('Location URL', 'candor'); ?></label>
					<input type="text" name="<?php echo $field['id']; ?>[<?php echo $key ?>][location_url]" value="<?php echo $pin['location_url']; ?>" />
				</fieldset>
				<fieldset class="pin_name">
					<label for="<?php echo $field['id']; ?>[<?php echo $key ?>][name]" ><?php _e('Name', 'candor'); ?></label>
	...
	<span class="button add_new_location"><?php _e('Add New Location', 'candor') ?></span>
