<div class="candor_builder_container hidden">
	<?php
	$base64_decode = false;
	$gridster_params = '';
	...
	echo '<input type="hidden" name="', $field['id'], '" id="candor_builder" value="', '' !== $meta ? htmlspecialchars( $meta ) : $content, '" ' . $gridster_params . ' ' . ( $base64_decode ? 'data-base64_encoded="true"' : '' ) .' />'; ?>
	<div class="pixbuilder-controls">
		<button class="add_block button button-primary button-large"
		        value="image"> <?php esc_html_e( '+ Add Image', 'candor' ); ?></button>
		<button class="add_block button button-primary button-large"
		        value="editor"> <?php esc_html_e( '+ Text', 'candor' ); ?></button>
	</div>

	<!-- <span class="clear-all button button-secondary">Clear All</span> -->

	<div class="candorbuilder-grid gridster">
		<ul>
			<?php
			if( ! empty ( $content ) ) {
					...
					foreach ( $content as $key => $block ) {
						...
						switch ( $block->type ) {
							case 'editor' :
								...
								$content = '<textarea class="to_send" style="display: none">' . $block->content. '</textarea>' . '<div class="editor_preview">' . '<div class="editor_preview_wrapper">' . candor_builder_display_content( $block->content, false ) . '</div>' . '</div>';

								$controls_content = '<a class="edit_editor"><span>' . esc_html__( 'Edit', 'candor' ) . '</span></a>';
								...
								if ( isset( $block->content ) ) {
									...
									if ( ! empty( $block->content ) && is_numeric( $block->content ) ) {
										...
										if ( isset( $attach[0] ) && ! empty( $attach[0] ) ) {
											...
											$controls_content = '<a class="open_media" href="#" class="wp-gallery" data-attachment_id="' . $block->content . '"><span>' . __( 'Set Image', 'candor' ) . '</span></a>';
										}
									} else {
										$content          = '<img class="image_preview">';
										$controls_content = '<a class="open_media" href="#" class="wp-gallery" data-attachment_id="' . $block->content . '"><span>' . __( 'Set Image', 'candor' ) . '</pan></a>';
									}
								}
								break;
							default :
						...
						<li id="block_<?php echo $block->id ?>" class="block-type--<?php echo $block->type; ?> item"
							...
							<div class="item__controls">
								<ul class="nav nav--controls">
									...
									<li class="position"><span><?php esc_html_e( 'Position', 'candor' ); ?></span>
										<div class="position__ui">
											<div
												class="position__ui-title"><?php esc_html_e( 'Alignment', 'candor' ); ?></div>
												...
												<div class="position__ui-row">
													<div
														...
														<div class="position__ui-handle"
														     data-step="<?php echo $block->position['top']; ?>"><?php esc_html_e( 'top', 'candor' ); ?></div>
													</div>
												</div>
												<div class="position__ui-row">
													<div
														...
														<div class="position__ui-handle"
														     data-step="<?php echo $block->position['left']; ?>"><?php esc_html_e( 'left', 'candor' ); ?></div>
													</div>
													...
													<div
														...
														<div class="position__ui-handle"
														     data-step="<?php echo $block->position['right']; ?>"><?php esc_html_e( 'right', 'candor' ); ?></div>
													</div>
												</div>
												<div class="position__ui-row">
													<div
														...
														<div class="position__ui-handle"
														     data-step="<?php echo $block->position['bottom']; ?>"><?php esc_html_e( 'bottom', 'candor' ); ?></div>
													</div>
												</div>
											</div>
										</div>
									...
									<li class="remove remove_block">
										<span><?php esc_html_e( 'Remove', 'candor' ); ?></span></li>
...
function my_admin_footer_function() { ?>
	<div class="candor_builder_editor_modal_container" style="display:none">
		<div class="modal_wrapper">
			<div class="media-modal wp-core-ui">
				...
				<div class="media-modal-content">
					<div class="media-frame-title"><h1><?php esc_html_e( 'Insert Content', 'candor' ); ?></h1></div>
					<div class="media-frame-router"></div>
					<div class="media-frame-content">
						<?php
						function candor_builder_change_mce_options( $initArray ) {
						...
						add_filter( 'tiny_mce_before_init', 'candor_builder_change_mce_options' );

						wp_editor( '', 'candor_builder_editor', array( 'textarea_rows' => 20, 'editor_height' => 350 ) );

						remove_filter( 'tiny_mce_before_init', 'candor_builder_change_mce_options' ); ?>
					</div>
					<div class="modal_controls media-frame-toolbar">
						<a class="close_modal_btn button button-large" href="#"><?php esc_html_e( 'Cancel', 'candor' ) ?></a>
						<a class="insert_editor_content button media-button button-primary button-large"
						   href="#"><?php esc_html_e( 'Insert Content', 'candor' ); ?></a>
					</div>
				</div>
			</div>
...
function candor_builder_display_content( $content = '', $decode = true ) {
