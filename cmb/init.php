...
class cmb_Meta_Box {
	...
	function show() {
		...
		foreach ( $this->_meta_box['fields'] as $field ) {
			...
			if ( ! ( $field['type'] == 'portfolio-gallery' || $field['type'] == 'gallery' || $field['type'] == 'candor_builder' || $field['type'] == 'gmap_pins' ) ) {
			...
			switch ( $field['type'] ) {
				...
				case 'candor_builder':
					...
					wp_enqueue_script( 'candor_builder' );
					wp_enqueue_style( 'candor_builder' );


					$file_path = plugin_dir_path( __FILE__ ) . 'fields/candor_builder.php';
					if ( file_exists( $file_path ) ) {
						ob_start();
						include( $file_path );
						echo ob_get_clean();
	...
	function fold_display() {
		// some special care for the candor_builder type
...
function cmb_scripts( $hook ) {
	...
	if ( $hook == 'post.php' || $hook == 'post-new.php' || $hook == 'page-new.php' || $hook == 'page.php' ) {
		...
		wp_register_script( 'candor_builder', CMB_META_BOX_URL . 'js/candor_builder.js', array( 'gridster' ), $plugin_version );
		wp_localize_script( 'candor_builder', 'l18n_candor_builder', array(
			'set_image' => __( 'Set Image', 'candor' ),
		) );
		...
		wp_register_style( 'candor_builder', CMB_META_BOX_URL . 'css/candor_builder.css', array( 'gridster' ), $plugin_version );
