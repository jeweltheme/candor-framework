 ...
 * The Shortcode
 */
function candor_shopaholic_blog_masonry_shortcode( $atts ) {
	extract( 
		shortcode_atts( 
			array(
				'grid_column' 	=> '2col',
								...
								<?php
									...
									<div class="<?php echo esc_attr($class); ?>">
										<?php
											 ...
											 */
											get_template_part( 'template-parts/masonry/content', get_post_format() );

										?>
										</div>
									<?php } } else{
										get_template_part( 'template-parts/masonry/content', 'none' );
									} 
...
<?php
	wp_enqueue_style( 'shopaholic-blog-masonry-style', SHOPAHOLIC_CSS . 'blog/masonry-01.css', false, SHOPAHOLIC_VER );
...
add_shortcode( 'shopaholic_blog_masonry', 'candor_shopaholic_blog_masonry_shortcode' );

/**
 * The VC Functions
 */
function candor_shopaholic_blog_masonry_shortcode_vc() {
	
	vc_map( 
		array(
			"icon" => 'shopaholic-vc-block',
			"name" => esc_html__("Blog Masonry", 'shopaholic-wp'),
			"base" => "shopaholic_blog_masonry",
...
add_action( 'vc_before_init', 'candor_shopaholic_blog_masonry_shortcode_vc');