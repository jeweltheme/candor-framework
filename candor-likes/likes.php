...
class candorLikes {
    ...
    function __construct() {	
        ...
        add_shortcode('candor_likes', array(&$this, 'shortcode'));
	}
	
	function enqueue_scripts(){
	    $options = get_option( 'candor_likes_settings' );
		...
		wp_localize_script( 'ebor-likes', 'candor_likes', array('ajaxurl' => admin_url('admin-ajax.php')) );
	}
	
	function setup_likes( $post_id ) {
		if(!is_numeric($post_id)) return;
	
		add_post_meta($post_id, '_candor_likes', '0', true);
	}
	...
	function like_this($post_id, $zero_postfix = false, $one_postfix = false, $more_postfix = false, $action = 'get'){
		...
		switch($action) {
			...
			case 'get':
				$likes = get_post_meta($post_id, '_candor_likes', true);
				if( !$likes ){
					$likes = 0;
					add_post_meta($post_id, '_candor_likes', $likes, true);
				}
			...
			case 'update':
				$likes = get_post_meta($post_id, '_candor_likes', true);
				if( isset($_COOKIE['candor_likes_'. $post_id]) ) return $likes;
				
				$likes++;
				update_post_meta($post_id, '_candor_likes', $likes);
				setcookie('candor_likes_'. $post_id, $post_id, time()*20, '/');
	...
	function do_likes(){
		...
		if( isset($_COOKIE['candor_likes_'. $post->ID]) ){
...
global $candor_likes;
$candor_likes = new candorLikes();

/**
 * Template Tag
 */
function candor_likes(){
	global $candor_likes;
    echo $candor_likes->do_likes(); 
}