...
class Candor_Options {
	...
	public function __construct(){
		add_action('customize_register', 'candor_additional_classes', 10 );
		...
		if( is_array($this->settings) ){
			foreach( $this->settings as $setting ){
				...
				if( 'image' == $type ){
					...
					$wp_customize->add_control( 
						new Candor_Customizer_Number_Control(
							$wp_customize, $name, array(
					    		'label'    => $title,
					    		'section'  => $section,
					    		'priority' => $priority
					...
					$wp_customize->add_control( 
						new Candor_Customizer_Textarea_Control(
							$wp_customize, $name, array(
					    		'label'    => $title,
					    		'section'  => $section,
					    		'priority' => $priority
					...
					$wp_customize->add_control( 
						new Candor_Customizer_Range_Control(
							$wp_customize, $name, array(
					    		'label'    => $title,
					    		'section'  => $section,
					    		'priority' => $priority,
					...
					$wp_customize->add_control( 
						new Candor_Customizer_Demo_Import_control( 
							$wp_customize, $name, array(
							    'label'    => $title,
							    'section'  => $section,
							    'priority' => $priority
...
if(!( function_exists('candor_additional_classes') )){
	function candor_additional_classes(){
		
		class Candor_Customizer_Range_Control extends WP_Customize_Control {
		    public $type = 'range';
		    public function render_content() {
		    ?>
		        <label>
		        	<span class="customize-control-title"><?php echo esc_html( $this->label ); ?></span>
		        	<input <?php $this->link(); ?> name="<?php echo esc_html( candor_sanitize_title($this->label) ); ?>" type="range" min="<?php echo $this->choices['min']; ?>" max="<?php echo $this->choices['max']; ?>" step="<?php echo $this->choices['step']; ?>" value="<?php echo intval( $this->value() ); ?>" class="ebor-range" onchange="printValue('<?php echo esc_html( candor_sanitize_title($this->label) ); ?>')" />
		        	<input type="text" name="<?php echo esc_html( candor_sanitize_title($this->label) ); ?>" class="ebor-range-output" value="<?php echo intval( $this->value() ); ?>" disabled/>
		        </label>
		    <?php
		    }
		}
		
		class Candor_Customizer_Textarea_Control extends WP_Customize_Control {
		...
		class Candor_Customizer_Number_Control extends WP_Customize_Control {
			public $type = 'number';
			public function render_content() {
			?>
				<label>
		...
		class Candor_Customizer_Demo_Import_control extends WP_Customize_Control {
